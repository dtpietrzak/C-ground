...
int handle_request_index(HttpRequest* http_request,
  ...
  char* index_path = derive_path_to_location("index", queries.db);
  ...
  int status = save_string_to_file(http_request->body.value, index_path);
  switch (status) {
    case -1: {
      // Check if the file exists
      char* file_access_issue = check_file_access(index_path, -2);
      ...
      http_response->status = 500;
      s_compile(&http_response->body, "%s", index_path);
      return 1;
    }
    case 0:
      http_response->status = 201;
      s_compile(&http_response->body, "\"Index inserted successfully: %s\"",
                index_path);
      return 0;
    case 1:
      http_response->status = 204;
      s_compile(&http_response->body, "\"Index updated successfully: %s\"",
                index_path);
      return 0;
    default: {
      // Check if the file exists
